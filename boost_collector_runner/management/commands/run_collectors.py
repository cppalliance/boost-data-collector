"""
Management command: run_collectors
Runs a fixed list of collector commands in order (see docs/Workflow.md).
Exits with 0 only when all succeed; non-zero on any failure.

For YAML-driven schedule use run_scheduled_collectors instead.
"""
...
# Fallback order when not using YAML (per Workflow.md)
COLLECTOR_COMMANDS = [
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        logger.info(
            "run_collectors: starting (%d commands)",
        ...
        logger.info(
            "run_collectors: finished; succeeded=%d, failed=%d",
            succeeded,
            failed,
        )
        self.stdout.write(
