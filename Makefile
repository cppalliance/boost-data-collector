...
.PHONY: health
health:
	$(COMPOSE) exec -T $(APP) sh -c 'curl -fsS http://127.0.0.1:8000/health/ | python -c "import sys,json; d=json.load(sys.stdin); print(d.get(\"status\")); sys.exit(0 if d.get(\"status\")==\"healthy\" else 1)"'
