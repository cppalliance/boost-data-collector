...
from operations.slack_ops.tokens import get_slack_bot_token, get_slack_app_token, get_slack_client
...
def test_get_slack_bot_token_from_env():
    ...
    with patch.object(settings, "SLACK_BOT_TOKEN", None):
        with patch.dict("os.environ", {"SLACK_BOT_TOKEN": "xoxb-from-env"}, clear=False):
...
def test_get_slack_app_token_from_env():
    ...
    with patch.object(settings, "SLACK_APP_TOKEN", None):
        with patch.dict("os.environ", {"SLACK_APP_TOKEN": "xapp-from-env"}, clear=False):
...
def test_get_slack_client_without_token_uses_get_slack_bot_token():
    ...
    with patch.object(settings, "SLACK_BOT_TOKEN", None):
        with patch.dict("os.environ", {"SLACK_BOT_TOKEN": "xoxb-env-token"}, clear=False):
