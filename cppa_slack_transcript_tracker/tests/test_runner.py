...
def test_run_slack_huddle_missing_bot_token_returns_without_starting():
    ...
    with patch("cppa_slack_transcript_tracker.runner.get_slack_bot_token", side_effect=ValueError("no token")):
        with patch("cppa_slack_transcript_tracker.runner.get_slack_app_token", return_value="xapp-t"):
            run_slack_huddle()
...
def test_run_slack_huddle_missing_app_token_returns_without_starting():
    ...
    with patch("cppa_slack_transcript_tracker.runner.get_slack_bot_token", return_value="xoxb-t"):
        with patch("cppa_slack_transcript_tracker.runner.get_slack_app_token", side_effect=ValueError("no app token")):
            run_slack_huddle()
...
def test_run_slack_huddle_with_tokens_starts_listener():
    ...
    with patch("cppa_slack_transcript_tracker.runner.get_workspace_root"):
        with patch("cppa_slack_transcript_tracker.runner.set_working_directory"):
            with patch("cppa_slack_transcript_tracker.runner.get_slack_bot_token", return_value="xoxb-t"):
                with patch("cppa_slack_transcript_tracker.runner.get_slack_app_token", return_value="xapp-t"):
                    with patch("cppa_slack_transcript_tracker.utils.slack_listener.start_slack_listener", MagicMock()) as mock_start:
                        run_slack_huddle()
